...
class MaskGIT:
    ...
    def prepare(self):
        ...
        for i in range(self.total_iter):
            os.makedirs(f"test_results/{self.mask_func}/iter{i + 1}", exist_ok=True)
    ...
    def inpainting(self,image,mask_b,i): #MakGIT inference
        ...
        with torch.no_grad():
            ...
            for step in range(self.total_iter):
                ...
                z_indices_predict, mask_bc = self.model.inpainting(ratio,self.mask_func,mask_num,mask_b,z_indices)
                ...
                vutils.save_image(dec_img_ori, os.path.join(f"test_results/{self.mask_func}/iter{step+1}", f"image_{i:03d}.png"), nrow=1)
...
#TODO3 step1-2: modify the path, MVTM parameters
    ...
    #MVTM parameter
    parser.add_argument('--sweet-spot', type=int, default=10000000, help='sweet spot: the best step in total iteration')
